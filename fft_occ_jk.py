...
#
# Modified from pbc/df/fft_jk.py
#
...
def get_k_kpts_occ(mydf, dm_kpts, hermi=1, kpts=np.zeros((1,3)), kpts_band=None,
    ...
    if gamma_point(kpts_band) and gamma_point(kpts):
        ...
        kiv = np.zeros((nset,nband,mo_coeff0[0].shape[1],nao), dtype=np.complex128)
    ...
    vR_dm = np.empty((nset,mo_coeff0[0].shape[1],ngrids), dtype=vk_kpts.dtype)
    ...
    for k2, ao2T in enumerate(ao2_kpts):
        ...
        if mo_coeff is None or nset > 1:
            ...
            #ao_dms = [ao2T.conj()]
            ao_dms = [ao2T.conj() / 1.41421356]
        for k1, ao1T in enumerate(ao1_kpts):
            kpt1 = kpts_band[k1]
            # occ
            ao3T = np.dot(mo_coeff[k1].T, ao1T)
            ...
            for p0, p1 in lib.prange(0, naoj, blksize):
            ...
            for i in range(nset):
                # occ
                kiv[i,k1] += weight * lib.dot(vR_dm[i], ao1T.T)
        # occ
        for k1, ao1T in enumerate(ao1_kpts):
            kij = lib.einsum('ui,ju->ij', mo_coeff0[0], kiv[i,k1])
